...
## 2026-09-27 WP-DUAL-LAYOUT-PLAYFLOW-CONTENT-01 · IMPLEMENTED / 本地验收进行中

从默认分支 `origin/main` 完整 SHA `0217871ca06a6b36d8cf0a34b7280a6f83283fe2` 的干净基线，在独立工作树及分支 `codex/dual-layout-playflow-content` 实现。现行合同是五章各五个营业小关、30 个主题小游戏关卡和 1 个混合复习；下文旧关卡数及旧布局 PASS 均只作历史证据。最终提交、同一干净构建的完整 SHA、PR 与命令退出码见本包 [证据清单](evidence/dual-layout-playflow/README.md) 和交付消息。

桌面、手机竖屏、短横屏和平板按可用空间使用独立编排，共用规则、Controller、存档与命中几何。冰淇淋的杯/筒、球数、口味、配料、启动及交付均由玩家明确操作；工作台和目的盘分开，食谱为按需参考。果汁、三明治、汉堡保留不同加工关系。错单仍保留食物，撤回与刷新保留实例身份；菜单暂缓后会在下一单重新提出，音频重听与接餐反馈按实际动作时序处理。

故事从旧食谱与钥匙开始，经过花、彩旗、桌布和灯串，最终由大咪完成交付并在小院合影。兔兔、刺刺、长辈有回访对白；第一、四章各有可改变先服务谁及回应的选择。历史五章收据保留，新增小关不自动标记完成。主题游戏各关有稳定 ID、目标、情境、题池、基础条件与 4–8 个任务；WordSpell/组句可选，未识字也可推进故事。只有餐车能做的食物进入营业请求。

本地自动化通过不等于负责人已接受新视觉与操作手感。实体 iPhone 15 Pro＋Chrome **BLOCKED / NEEDS_OWNER_REVIEW**；儿童观察、教研、人工听审 **NOT_RUN**；新对白沿用已有开发语音/文字，**未新增声音或图片**，既有资源权利/听审仍 **PENDING**。未合并、未部署；公开站点版本与本包验收 **PENDING_DEPLOYMENT**。
...
第二轮 [CI](https://github.com/yuan2go/tabby-english-food-truck/actions/runs/36247345796) 在 `6208e015da3a63ee5c4995fad009d4794d414093` 上前置关卡通过、浏览器 **6/11 退出 1**：处理过程中又暴露前置 `processing` 和满盘物品数量假设，声音失败时开场需显式走“看图继续”；Linux 软渲染下连续走查与拼词触摸长时间占用浏览器。现检查合法的加工前后状态与实例守恒，失败声音按实际界面继续；CI 通过产品已有省电画面设置降低像素预算，仅保留失败录像，默认画质仍由本地原有证据覆盖。修订后本地同组 Chromium **11/11、退出 0**，见本包日志。此处记录的是分轮结果，不把本地省电模式等同真机/默认画质验收；最终远端结论看 PR 最新 head 检查。
