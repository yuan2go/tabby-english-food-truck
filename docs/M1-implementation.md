...
- [ ] 规则/快照：catalog 定义 guided/practice/service；createGame 依据模式创建内容与并发，单盘约束进入规则及 snapshot；按模式保存恢复，累计跨模式支持。advance 改结构共享避免每帧全状态克隆。测试三模式完成、切换、坏档、守恒。
- [ ] 表现/输入：TruckScene 按实体 ID 持有图像；物品局部运动、整盘相对坐标、助手实际来源和绑定槽位；Note 按插入边界校正前移/后移，统一单指捕获和取消。测试连续命令、混合取料、重复 token。
- [ ] 制作：参考驱动独立高清猫姿态，原稿与运行导出分层；逐姿态登记 Alpha/有效像素/引用。场景内先实物语音示范再动手，图片助手与可选词块共享规则。
- [ ] 清晰度/音频：测 baseline DPR 与缓冲，核对 Phaser 3.90 官方源码；限定像素预算，高分辨率文字与一致坐标。统一音频分层、duck、PROCESSING 循环、设置保存与生命周期清理。
- [ ] 验证/交付：现有 npm scripts、真触控低龄与双单完整视频、Pad、性能采样、资源检查、未登录站点；更新此清单和 STATUS，阶段提交/push/PR，不合并。
