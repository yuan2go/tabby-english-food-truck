  ...
  for (const name of (await readdir(folder)).sort()) {
    ...
    const base = {
      ...
      derivation: reference
        ...
        : runtime && !sound
          ? id === 'vanilla-cup'
            ? 'Reference edit of strawberry-cup; source unchanged, proportional downscale only; art/m22-vanilla-cup.json'
            : id.startsWith('food-')
              ? 'scripts/export-m22.mjs: independent original alpha crop and downscale only; art/m22-production.json'
              : cat
...
await writeFile(
  'art/manifest.json',
  `${JSON.stringify({ version: 'm2.2', assets: entries }, null, 2)}\n`,
);
console.log(
